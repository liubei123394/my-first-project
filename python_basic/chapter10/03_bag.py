#包概述:
#1.在Python 中，【包含init,py 的文件夹】就是一个包(Package)
...
#2.import 包名.模块名块名 as 别名
#import trade.order as dd
#import trade.pay as zf

#dd.create_order()
#zf.wechat_pay()
#3.from 包名.模块名块名 import 具体内容1，具体内容2,………
...
#order.create_order()
#pay.wechat_pay()
#7.from包名import模块名as别名
#from trade import order as dd, pay as p
#dd.create_order()
#p.wechat_pay()

#关于init .py 文件:
#1._init.py 是包的初始化文件，在包被导入时，_init.py 会被自动调用
#2.init.py 中可以编写一些包的初始化逻辑
#3._init.py 中所定义的内容，会被【from 包名 import*】形式全部引入
#4._init_.py 中也可以使用a11来控制包中的哪些模块可以被【from 包名 import*】引入
