#装饰器
#1.装饰器是一种【可调用对象】（通常函数），他能接收一个函数作为参数，并且会返回一个新函数
#2.装饰器可以在不修改原函数代码的前提下，增强或改变原函数的功能

#关键点：
#1.接受被装饰的函数，同时返回新函数（wrapper）
#2.装饰器“吐出来”的是 wrapper 函数，以后别人调用的也是 wrapper函数
...
#需求：在不修改add函数的前提下，给add函数增加一些额外的功能，例如：打算前先打印一句欢迎语
#实现方案：使用装饰器
...
#进阶：带参数的装饰器（三层嵌套，外层接受配置，中间层接收函数，内层接受具体参数）
def say_hello(msg):
    def outer(func):
        def wrapper(*args, **kwargs):
